...
6. Use `unlazy` for substantial accepted work where forgotten deliverables or premature completion are realistic. It cannot override another skill's `AWAITING_APPROVAL`, safety, or release boundary.
7. Use `multi-agent-work-coordinator` only when work can be decomposed into non-overlapping ownership with explicit integration order.
8. Use `verification-and-release` for final integrated ship/no-ship judgment, not as a generic test runner.
9. Preserve repository-local `AGENTS.md` instructions. More specific project rules override this generic routing guidance.
